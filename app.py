...
VIVI_MODEL_ID = "llama-3.3-70b-versatile"
...
def _vivi_generate(messages: list, system_prompt: str) -> str:
    ...
    api_key = os.environ.get("GROQ_API_KEY", "").strip()
    if not api_key:
        api_key = __import__("os").environ.get("GEMINI_API_KEY", "") or st.secrets.get("GROQ_API_KEY", "")
    if not api_key:
...
/* ── Botões dentro de expanders: ícone quadrado 36px ── */
...
(function(){
    ...
    var PROPS = [
        ...
        ['text-transform','uppercase'],['min-height','36px'],['height','36px'],
        ['line-height','36px'],
        ['display','inline-flex'],['align-items','center'],['justify-content','center'],
        ['gap','0.3rem'],['cursor','pointer'],['white-space','nowrap'],
        ['word-break','keep-all'],['padding','0 0.75rem']
    ];
...
if _sidebar_open:
    ...
    st.markdown("""
    <style>
    div[data-testid="stButton"]:has(button[data-testid="baseButton-secondary"]#btn_abrir_menu_) button,
    ...
    """, unsafe_allow_html=True)
    st.markdown('<div class="btn-abrir-menu">', unsafe_allow_html=True)
    if st.button("☰", key="btn_abrir_menu", help="Abrir menu"):
        st.session_state["sidebar_open"] = True
        st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with _top_close:
        ...
        .btn-fechar-sb button:hover {
            border-color: #00f5ff !important;
            color: #00f5ff !important;
            box-shadow: 0 0 8px rgba(0,245,255,0.35) !important;
            background: rgba(0,245,255,0.07) !important;
        }
        </style>
        """, unsafe_allow_html=True)
        st.markdown('<div class="btn-fechar-sb">', unsafe_allow_html=True)
        if st.button("«", key="btn_fechar_sidebar", use_container_width=True, help="Fechar menu"):
                ...
                with st.expander(exp_label):
                    ...
                    if links_do_produto:
                        ...
                        for _lk_idx, lk in enumerate(links_do_produto):
                            ...
                            _l1, _l2, _l3, _l4, _l5 = st.columns([4, 1, 1, 1, 1], vertical_alignment="center")
                            with _l1:
                                st.markdown(
                            ...
                            with _l3:
                                st.markdown('<div class="btn-wrap-cyan">', unsafe_allow_html=True)
                                if st.button("⚡", key=f"sl_{lk.id}", help="Encurtar link", use_container_width=True):
                            ...
                            with _l4:
                                st.markdown('<div class="btn-wrap-pink">', unsafe_allow_html=True)
                                if st.button("✏️", key=f"edit_lk_{lk.id}", help="Editar link", use_container_width=True):
                            ...
                            with _l5:
                                st.markdown('<div class="btn-wrap-red">', unsafe_allow_html=True)
                                if st.button("🗑️", key=f"dl_{lk.id}", help="Excluir link", use_container_width=True):
                            ...
                            if bitly_url:
                                if str(bitly_url).startswith("ERRO:"):
                                    ...
                                    _bl1, _bl2, _bl3, _bl4, _bl5 = st.columns([4, 1, 1, 1, 1], vertical_alignment="center")
                                    with _bl1:
                                        st.markdown(
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    with col_list:
        ...
        st.markdown("""
        <style>
        /* ── Botões de ícone do Gerador de Links — borda sempre visível ── */
        .gl-icon-btn button {
        ...
        .gl-icon-btn-edit button {
            border: 2px solid rgba(244,114,182,0.6) !important;
            color: #f472b6 !important;
            box-shadow: 0 0 6px rgba(244,114,182,0.2) !important;
        }
        .gl-icon-btn-edit button:hover {
            border-color: #f472b6 !important;
            box-shadow: 0 0 14px rgba(244,114,182,0.5) !important;
            background: rgba(244,114,182,0.08) !important;
        }
        .gl-icon-btn-del button {
            border: 2px solid rgba(255,0,127,0.5) !important;
            color: #ff6b9d !important;
            box-shadow: 0 0 6px rgba(255,0,127,0.15) !important;
        }
        .gl-icon-btn-del button:hover {
        ...
        if _gl_links:
            ...
            for lk in _gl_links:
                ...
                # ── Linha: URL + badge + copiar + editar + deletar ────────────
                _r1, _r2, _r3, _r4 = st.columns([5, 1, 1, 1], vertical_alignment="center")

                with _r1:
                    _lk_label = f" · {lk.rotulo}" if lk.rotulo else ""
                    st.markdown(
                        f'<div style="display:flex;align-items:center;gap:0.5rem;'
                        f'padding:0.3rem 0;flex-wrap:nowrap;min-width:0;">'
                        f'<span style="font-family:\'Rajdhani\',sans-serif;font-weight:700;'
                        f'font-size:0.88rem;color:#00f5ff;text-shadow:0 0 8px rgba(0,245,255,0.5);'
                        f'white-space:nowrap;">'
                        f'?r={lk.url_encurtada}</span>'
                        f'<span class="badge badge-cyan" style="font-size:0.6rem;white-space:nowrap;">'
                        f'{lk.cliques} cliques</span>'
                        f'{"<span style=\"font-size:0.72rem;color:#4a5a80;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;\">" + lk.rotulo + "</span>" if lk.rotulo else ""}'
                        f'</div>',
                        unsafe_allow_html=True,
                    )
                ...
                with _r3:
                    st.markdown('<div class="gl-icon-btn gl-icon-btn-edit">', unsafe_allow_html=True)
                ...
                with _r4:
                    st.markdown('<div class="gl-icon-btn gl-icon-btn-del">', unsafe_allow_html=True)
                ...
                st.markdown('<hr style="border:none;border-top:1px solid rgba(0,245,255,0.07);margin:0.4rem 0;">', unsafe_allow_html=True)
        else:
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    _gemini_key = st.secrets.get("GEMINI_API_KEY") or st.secrets.get("GROQ_API_KEY", "").strip()
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    with col1:
        ...
        _cur_key = st.secrets.get("GROQ_API_KEY", "")
        _key_status = (
...
# ── Estiliza os botões nativos de colapso/expansão do Streamlit ──────────────
st.markdown("""
<style>
/* Botão « (fechar) — nativo do Streamlit, ESCONDIDO */
[data-testid="stSidebarCollapseButton"] {
    display: none !important;
}

/* Botão ☰ (abrir) — visível, funcional, estilizado */
[data-testid="collapsedControl"] {
    visibility: visible !important;
    display: flex !important;
    opacity: 1 !important;
...
[data-testid="collapsedControl"] button:hover {
    background: rgba(0,245,255,0.12) !important;
    border-color: #00f5ff !important;
    box-shadow: 0 0 18px rgba(0,245,255,0.55) !important;
}
